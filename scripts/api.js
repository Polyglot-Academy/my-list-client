...
class FakeAPI {
  ...
  initializeData() {
    ...
    if (!localStorage.getItem("mylist_categories")) {
      const categories = [
        { id: 1, usuario_id: 1, nome: "Trabalho", criado_em: new Date().toISOString() },
        { id: 2, usuario_id: 1, nome: "Pessoal", criado_em: new Date().toISOString() },
        { id: 3, usuario_id: 1, nome: "Compras", criado_em: new Date().toISOString() },
      ]
  ...
  async getCategories() {
    await this.delay()
    const categories = JSON.parse(localStorage.getItem("mylist_categories") || "[]")
  ...
  async deleteCategory(id) {
    ...
    const updatedTasks = tasks.map((t) => (t.categoria_id === id ? { ...t, categoria_id: null } : t))

    localStorage.setItem("mylist_categories", JSON.stringify(filteredCategories))
  ...
  async getTasks(filters = {}) {
    ...
    if (filters.categoria_id) {
      userTasks = userTasks.filter((t) => t.categoria_id === Number.parseInt(filters.categoria_id))
    }
  ...
  async createTask(taskData) {
    ...
    const newTask = {
      ...
      usuario_id: currentUser.id,
      categoria_id: taskData.categoria_id ? Number.parseInt(taskData.categoria_id) : null,
